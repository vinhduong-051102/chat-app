...
const ListUserChat: React.FC<propsTypes> = ({ data }) => {
  ...
  const transformDisplayHours = useCallback(
    (index: number) => {
      if (data.length > 0) {
  ...
  const transformDisplayMinutes = useCallback(
    (index: number) => {
      if (data.length > 0) {
        const createdAt = data[index].createdAt;
                    ...
                    <Space size='middle'>
                      ...
                      <Col span={24}>
                        ...
                        <div className={cx("latest-message")}>
                          {dotdotdot(item.latestMessage, 14)} {`${transformDisplayHours(index)}:${transformDisplayMinutes(index)}`}
                        </div>
                      </Col>
                    </Space>
                  </Row>
